describe('Epic 1: Review', () => {
    ...
    it ('should create review', () => {
        ...
        cy.reload();
        cy.get('[data-cy=reviewCard]').get('[data-cy=cardText]').contains('This is a test review');
    });

    it ('should delete review', () => {
        cy.get('[data-cy=reviewCard]').first().get('[data-cy=deleteReview]').click();
        cy.reload();
        cy.get('[data-cy=reviewCard]').first().get('[data-cy=cardText]').contains('This is a test review').should('not.exist');
    });
});